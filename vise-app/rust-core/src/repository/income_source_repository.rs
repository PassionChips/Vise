...
use crate::models::income_source::{
    IncomeSource,
    NewIncomeSource,
    UpdateIncomeSource,
};

pub fn insert(connection: &mut SqliteConnection, source:&NewIncomeSource)->QueryResult<IncomeSource>{
...
pub fn delete(
    ...
    let affected_rows = diesel::delete(income_sources::table.filter(income_sources::id.eq(source_id)))
    .execute(connection)?;
...
use super::*;
    ...
    fn sample_income_source(name:String)->NewIncomeSource{
        NewIncomeSource { name, is_active:true }
    }
    ...
    fn get_all_income_source_test(){
        ...
        insert(
            &mut connection,
            &sample_income_source("Salary".to_string()),
        )
        .unwrap();
